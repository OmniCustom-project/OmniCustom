...
def preprocess_audio_tensor(audio_path, device, target_dtype, sr=16000, clip_len=None, normalize=True):
    ...
    wave_data, sample_rate = librosa.load(audio_path, sr=sr, mono=True)  # [-1, 1]
    assert (sample_rate == sr)
    if normalize:
        wave_data = wave_data / (np.max(np.abs(wave_data)) + 1e-6) * 0.95  # align with mmaudio/data/extraction.wav_dataset.py line 89
